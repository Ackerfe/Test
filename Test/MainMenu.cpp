	...
	Ackerfe::OSInterface* window, Ackerfe::GUI* gui, Ackerfe::InputHandler* input)
{
	...
	std::string tempString = "SoundEffects/Music/IWasTheSun.mp3";
	mSystem->createSound(tempString.c_str(), FMOD_DEFAULT, NULL, &newSound);
...
void MainMenu::mainMenuLoop()
{
	...
	Ackerfe::SpriteFont spriteFont("Fonts/astron boy.ttf", 64);
	char buffer[256];
	...
	CEGUI::PushButton* startButton = (CEGUI::PushButton*)mGUI->createWidget("TaharezLook/Button", glm::vec4(0.4f, 0.3f, 0.2f, 0.1f), glm::vec4(0.5f), "StartButton");
	...
	mSystem->playSound(newSound, NULL, false, NULL);
